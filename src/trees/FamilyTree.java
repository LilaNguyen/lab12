...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			...
			throw new TreeException("Line is illegal (has no colon): " + line);
		...
		else
		{
			...
			if (parentNode == null) {
				throw new TreeException("Parent node not found. Something wrong in the data file.");
			}
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			// ??? Throw a TreeException with a useful message
			throw new TreeException("No tree node with the name " + name1);
		TreeNode node2 = root.getNodeWithName(name2);	// node whose name is name2
		if (node2 == null)
			// ??? Throw TreeException with a useful message
			throw new TreeException("No tree node with the name " + name2);
